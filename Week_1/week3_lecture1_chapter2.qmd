...
```{r}
likelihood_article_fake_has_exc <- 0.6
```
...
```{r}
(prior_probability_has_exc * likelihood_article_fake_has_exc)/prior_probability_has_exc
```
...
#fake_news %>% (table/data)
#  tabyl(title_has_excl) %>%  (pull data)
#  adorn_totals("row") 
...
ggplot(data=pop_vs_soda, aes(x = region, fill = pop)) +
  geom_bar()
#Survey cocentration is centered in Midwest
#Possible bias
```
...
3\) Marginal Prior Probability that someone uses the word "pop"

```{r}
prior_probability_says_pop <- sum(priors*likelihood)
prior_probability_says_pop
```

```{r}
(priors[3] * likelihood
  [3])/ prior_probability_says_pop
```
