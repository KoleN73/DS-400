...
first \# (alpha) is for support, 2nd \# (beta) is for against support of candidate, the higher the the #s, the less standard deviation is. Sample size matters, the larger, the more certain answer that can be given. Dotted line is the mode, the solid line is the mean.

![](images/beta-tuning-1.png)

![](images/beta_param_quiz.png){width="771" height="255"}

1\) B, same value is symmetry

2\) A, Greater alpha value causes more support, skews to higher rating, skews right.

3\) Beta (5,5) Wider shape, greater uncertainty
...
Prior updated, posterior will be between likelihood and prior
...
```{r}
plot_beta_binomial(alpha = 90, beta = 2, y = 10, n = 40)
```
