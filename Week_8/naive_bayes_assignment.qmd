...
```{r}
#Type
ggplot(data = fake_news, aes(x = title_words, y = negative, color = type, shape = title_has_excl)) +
  geom_point()+
...
**Using the visual provided above, the line of best fit seems to indicate that the new news article is true, as the line of best fit for true is closer than the false line of best fit. It should be noted that along the 15 title word mark of the x-axis, there are more negative values of fake articles than there is of real news.**
...
predict(naive_model_hints, newdata = new_news_article, type = "raw")

```{r}
naive_model_hints_fn <- naiveBayes(type ~ title_words + negative + title_has_excl, data = fake_news)
```

```{r}
new_news_article <- data.frame(title_words = 15, negative = 6, title_has_excl = "false")
```

```{r}
predict(naive_model_hints_fn, newdata = new_news_article, type = "raw")
```

```{r}
naive_model_hints_fn #Run in console
```
...
-   Multiply these probabilities and save as the object **`probs_real`**
    ...
    ```{r}
    probs_real <- (0.04436553*0.02452492*0.9777778)
    print(probs_real)
    ```
...
```{r}
probs_fake <- ( 0.0826183*0.01642085*0.7333333 )
print(probs_fake)
```
...
```{r}
fake_news <- fake_news %>%
  mutate(predicted_type = predict(naive_model_hints_fn, newdata = .))
view(fake_news)
```
...
```{r, message=FALSE, warning=FALSE}
naive_model_fake_news_2 <- naiveBayes(type ~ ., data = fake_news)
predict(naive_model_fake_news_2, newdata = new_news_article, type = "raw")
```
