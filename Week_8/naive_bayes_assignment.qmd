...
ggplot(fake_news, aes(x = type, y = percentage, fill = title_has_excl)) +
...
#ggplot(data = fake_news_percent, aes(x = type, fill = title_has_excl)) +
#  geom_bar()
...
```{r}
ggplot(data = fake_news, aes(x = above_average_weight, fill = species)) +
  geom_bar()
```
